...
from orbis.data.schema import Oracle, Test
...
class CGCRepair(BenchmarkHandler):
    ...
    def set(self):
        self.env["CGC_INCLUDE_DIR"] = self.get_config('paths')['include']
        lib_path = self.get_config('paths')['libs'][32 if "M32" in self.env else 64]
    ...
    def make(self, context: Context, handler: MakeHandler, write_build_args: str = None,
        ...
        cmd_data = CommandData(args=f"cmake {cmake_opts} {context.root} -DCB_PATH:STRING={context.program.name}",
        ...
        if write_build_args:
            ...
            commands = handler.get_cmake_commands(working_dir=context.root, src_dir=context.src, skip_str="-DPATCHED",
                                                  build_dir=context.build, compiler_trail_path=compiler_trail_path)

            handler.write_cmake_build_args(dest=Path(write_build_args), vuln_files=context.program.vuln_files,
    ...
    def build(self, context: Context, handler: BuildHandler, coverage: bool = False, fix_files: List[AnyStr] = None,
        ...
        cmake_source_path = context.build / context.program.name / "CMakeFiles" / f"{context.program.name}.dir"

        # Backups manifest files
        if backup:
            cmd_data = handler.backup_manifest_files(out_path=Path(backup), source_path=context.source,
                                                     manifest_files=context.program.vuln_files)

        if link:
            cmd_data = handler.cmake_link_executable(source_path=context.source, cmake_path=cmake_source_path,
                                                     build_path=context.build / context.program.name)
        elif inst_files:
            mappings = context.program.map_files(inst_files, replace_ext=('.c', '.i'), skip_ext=[".h"])
            cmake_commands = handler.get_cmake_commands(working_dir=context.root, src_dir=context.src,
            ...
            cmd_data = handler.cmake_build(target=context.program.name, cwd=str(context.build),
                                           env=self.env)
    ...
    def test(self, context: Context, handler: TestHandler, tests: Oracle, timeout: int, neg_pov: bool = False,
        ...
        bin_names = get_binaries(context.source, binary=context.build / context.program.name / context.program.name)
        test_outcomes = []
        tests.args = f"{tests.args} --directory {context.build / context.program.name} --concurrent 1 --debug " \
